...
export DJANGO_SETTINGS_MODULE=tempracatao.settings.test
endif
...
lint: _flake8 _isort _black _dead_fixtures _mypy detect_missing_migrations validate_openapi_schema  ## Check code lint
...
run-dev: ## Run server with development settings
	@python src/manage.py migrate --no-input --settings tempracatao.settings.development
	@python src/manage.py runserver --settings tempracatao.settings.development
